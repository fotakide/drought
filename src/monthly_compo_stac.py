...
def check_gri_refinement(items):
    ...
    for item in items:
        ...
        if datastrip_metadata_url:
            # _log.info(f"Processing: {datastrip_metadata_url}")
            logging.info(f"Processing: {item.id}") #_log.info(f"Processing: {datastrip_metadata_url}")
            ...
            if xml_response.status_code == 200:
                ...
                logging.info(f"Image Refining Flag: {refining_flag}")
                ...
                # _log.info(f"Failed to fetch XML: {xml_response.status_code}")
                refinement_data.append({
                    "item_id": item.id,
                    "refinement_status": "Fetch Failed"
                })
        else:
            # _log.info("datastrip_metadata.xml not found in STAC item")
            refinement_data.append({
                "item_id": item.id,
...
def generate_composite(year_month: str, AOI_path: str):
    # Set up logger.
    _log = setup_logger(logger_name='compgen_',
                                logger_path=f'../logs/compgen_{datetime.datetime.now(pytz.timezone("Europe/Athens")).strftime("%Y%m%dT%H%M%S")}.log', 
                                logger_format='%(asctime)s - %(levelname)s - %(filename)s:%(lineno)d - %(message)s',
                                )
    
    AOI_number = int((AOI_path).split('AoI')[1].split('.')[0])
    ...
    if (os.path.isfile(f"{FOLDER_NAME}/{DATASET_NAME}.tif")) or (os.path.isfile(f"{FOLDER_NAME}/{DATASET_NAME}_NOTREFINED.tif")):
        msg = f"This composite already exists in {FOLDER_NAME}/{DATASET_NAME}.tif | Continuing."
        _log.warning(msg)
        return
    ...
    aoi_bbox = aoi_bbox.buffered(xbuff=0.05, ybuff=0.05) #buff by 0.05 degrees
    ...
    AOI_number = int((AOI_path).split('AoI')[1].split('.')[0])
    _log.info(f'-----------------------------------')
    _log.info(f'Operating for: AOI {AOI_number} | Time: {year_month}')
    ...
    search = catalog.search(
        ...
        query={
            ...
            "s2:nodata_pixel_percentage": {"lt":33}, #Avoid diagonal images
        },
    )

    items = search.item_collection()
    _log.info(f'Found {len(items)} items')
    if len(items) == 0:
        msg = f"AOI {AOI_number} | Time: {year_month}: Search returned 0 Items for `cloud cover={cloud_cover}`. Skipping this composite generation and continuing."
        _log.warning(msg)
        # raise ValueError(msg)
        return # Exits the function without breaking the entire process
    
    _log.info(f'Searching for GRI REFINED scenes...')
    _log.info(f'###################################')
    ...
    _log.info(f'{len(refined_items)}/{len(df_refinement_status)} were refined by GRI.')
    
    # items = refined_items

    refineflag = None
    if len(refined_items) == 0:
        msg = f"AOI {AOI_number} | Time: {year_month}: All scenes are flagged NOT REFINED."
        _log.warning(msg)
        _log.warning(f"AOI {AOI_number} | Time: {year_month}: The composite will be flagged with _NOREFINED.")
        refineflag = 'NOTREFINED'
        refined_items = items

    filtered_items = []
    tiles = np.unique([i.properties['s2:mgrs_tile'] for i in refined_items])
    epsgs = np.unique([i.properties['proj:epsg'] for i in refined_items])
    
    for tile in tiles:
        item_mgrs_sorted = sorted([i for i in refined_items if i.properties['s2:mgrs_tile'] == str(tile)], key=lambda item: item.properties['eo:cloud_cover'])
        if len(item_mgrs_sorted) > 18:
            filtered_items.extend(item_mgrs_sorted[:18])
        else:
            filtered_items.extend(item_mgrs_sorted)
    _log.info(f"Filtered cleanest scenes: Kept {len(filtered_items)} out of {len(refined_items)} items. ")
    _log.info(f"Tiles: {tiles}")
    _log.info(f"EPSG: {epsgs[0]}")
    # items = filtered_items
    _log.info(filtered_items)

    _log.info(f'###################################')
    _log.info(f'Lazy loading STAC items from Planetary Computer')
    ...
    _log.info(f'Masking bits: {invalid_scl_values}')
    ...
    try:
        _log.info(f'Downloading bands and computing median composites: AOI {AOI_number} | Time: {year_month}')
        median_composite = ds_cube_cf[bands].median(dim='time').astype('float32').compute()
        
        # bands = [b for b in list(median_composite.keys()) if b.startswith('B')]
        # ds_selected = cube_3Is_stretched[bands].isel(time=0)
        _log.info('Convert to unsigned integer 16-bits')
        ...
        _log.info(f'Write composite to GeoTIFF -> {FOLDER_NAME}/{DATASET_NAME}.tif')
        ...
        msg=f'Failed loading for : AOI {AOI_number} | Time: {year_month}'
        _log.error(msg)
        client.close()
        cluster.close()
        return
    ...
    for json_file in json_files:
        try:
            with open(json_file) as f:
                ...
                AOI_path = parameters_dict['AOI_path']

                generate_composite(year_month=year_month, AOI_path=AOI_path)
